...
void loop() {
  ...
  if (fabs(xf) < 0.12) xf = 0;
  if (fabs(yf) < 0.12) yf = 0;
  ...
  if (!holding && clickCount > 0 && !clickResolved) {
    ...
    if (millis() - lastRelease > CLICK_GAP) {
      ...
      if (clickCount == 1) msg = "Single";
      else if (clickCount == 2) msg = "Double";
      else if (clickCount == 3) msg = "Triple";
      else msg = "Multi";
  ...
  lcd.setCursor(0, 0);
  char buf[17];
  snprintf(buf, sizeof(buf), "X:%+5.2f Y:%+5.2f", xf, yf);
  lcd.print(buf);
